    ...
    <div class="UserLec_DetailsBackground">
        <div class="UserLec_Details">
            <div class="UserLec_UserImage">
                <?php
                $userID = $_SESSION['userName'];
                $result = $con -> query("SELECT * FROM lecturer WHERE UserID='$userID'") OR die(mysqli_error());
            ...
            <div class="UserLec_EditButton">
                <button type="button" class="UserLec_editButton" onclick="userProfileLec_edit()">Edit</button>
        ...
        <div class="UserLec_Notice">
            ...
            <form action="PHP/notice.php" method="post" id="UserLec_FormNotice" onsubmit="UserLec_onSubmit()">
                <div class="UserLec_NoticeForm">
                    <div>
                        <input name="group" pattern="Y[1-4]S[1-2]+ +[1-12]" id="UserLec_Group" class="UserLec_InputBox" type="text" placeholder="Group (Y1S1 2)" required>
                        <textarea name="notice" id="UserLec_Notice" class="UserLec_NoticeD" type="text" placeholder="Notice" required></textarea>
                    </div>
                    <div>
                        <div>
                            <label class="UserLec_Radio">
                                <input type="radio" value="FOC" name="notice_type" required>
                                FOC
                            </label>
                            <label class="UserLec_Radio">
                                <input type="radio" value="FOE" name="notice_type" required>
                                FOE
                            </label>
                            <label class="UserLec_Radio">
                                <input type="radio" value="FOB" name="notice_type" required>
                                FOB
                            </label>
                            <p id="UseLec_checkBox"><input id="UseLec_checkBoxS" onclick="UserLec_enableButton()" type="checkbox" required>All the information are true and correct</p>
                        </div>

                        <button type="submit" id="UserLec_postBtn" disabled>POST</button>
                    </div>
                </div>
            </form>
        </div>
            ...
            for ($i = 0; $i < 4; $i++) {
                ...
                for($j = 0; $j < 5; $j++){
                    $result = $con -> query("SELECT * FROM timeslots WHERE Day='$day[$j]' AND Time='$time[$i]' AND lecID='$lecID'") OR die(mysqli_error());
                    if(mysqli_num_rows($result) > 0){
                    while ($row = mysqli_fetch_array($result)) {
                        echo "<td id=\"dataTabl\">".$row['module']."<br>".$row['hall']."<br>".$row['groupID']."</td>";
                    }
                } else {
            ...
            ?>
            <br>
            <button class="UserLec_bnDownload" id="UserLec_btnDownload" onclick="downloadLec()">Download</button>
        </div>
    </div>
...
<form  action="PHP/changePasswordLec.php" method="post">
<div class="">
