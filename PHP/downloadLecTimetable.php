...
$details = $con -> query("SELECT * FROM lecturer WHERE UserID='$user'") OR die(mysqli_error());
if(mysqli_num_rows($details) > 0) {
    while ($row = mysqli_fetch_array($details)) {
        $fname = $row['LecturerName'];
    }
}

$pdf -> Ln();
$pdf -> cell(100,7,'LECTURER ID : '.$user, 0,1);
$pdf -> cell(100,7,'NAME : '.$fname,0,1);
...
for ($i = 0; $i < 4; $i++) {
    ...
    for($j = 0; $j < 5; $j++){
        $result = $con -> query("SELECT * FROM timeslots WHERE Day='$day[$j]' AND Time='$time[$i]' AND lecID='$user' ") OR die(mysqli_error());
